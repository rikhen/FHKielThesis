...
Technisch basiert dieser Teil auf \texttt{BibTeX} \parencite{Patashnik1988}
\bzw\ \texttt{Biber}%
\footnote{Wird seit Version 2013/02/19 anstelle von \texttt{bibtex} verwendet 
	(s.\ \url{http://biblatex-biber.sourceforge.net/}),}
in Kombination mit dem Paket \texttt{biblatex} \parencite{Lehman2016}.
...
Um einen Eintrag im Quellenverzeichnis zu erstellen und im Text darauf zu verweisen, stellt BibLaTeX zwei Kommandos zur Verfügung.

\subsection{Die \texttt{\textbackslash parencite}, \texttt{\textbackslash textcite} und \texttt{\textbackslash cite} Makros}

Für Quellenverweise im laufenden Text wird die Anweisung
\begin{itemize}
\item[] \verb!\parencite{!\textit{Verweise}\verb!}! 
				\quad oder \quad
        \verb!\parencite[!\textit{Zusatztext}\verb!]{!\textit{Verweise}\verb!}!.
\end{itemize}

\noindent%
verwendet. \textit{Verweise} ist eine durch Kommas getrennte Auflistung von $1$--$n$ Quellen-\emph{Schlüsseln}
...
\begin{LaTeXCode}[numbers=none]
Mehr dazu findet sich in \parencite{Kopka2003}.
\end{LaTeXCode}
$\rightarrow$ Mehr dazu findet sich in \parencite{Kopka2003}.

\begin{LaTeXCode}[numbers=none]
Mehr über \emph{Styles} in \parencite[Kap.\ 3]{Kopka2003}.
\end{LaTeXCode}
$\rightarrow$ Mehr über \emph{Styles} in \parencite[Kap.\ 3]{Kopka2003}.

\begin{LaTeXCode}[numbers=none]
Die Angaben in \parencite[S.\ 274--277]{BurgeBurger1999} sind falsch.
\end{LaTeXCode}
$\rightarrow$ Die Angaben in \parencite[S.\ 274--277]{BurgeBurger1999} sind falsch.

\begin{LaTeXCode}[numbers=none]
Überholt sind auch \parencite{BurgeBurger1999, Patashnik1988, Duden1997}.
\end{LaTeXCode}
$\rightarrow$ Überholt sind auch \parencite{BurgeBurger1999, Patashnik1988, Duden1997}.

Die Sortierung der Angaben im letzten Beispiel erfolgt automatisch.

Soll explizit der Name des Autors in Verbindung mit dem Quellenverweis verwendet werden, so
kommt die folgende Anweisung zum Einsatz:

\begin{itemize}
	\item[] \verb!\textcite{!\textit{Verweise}\verb!}! 
	\quad oder \quad
	\verb!\textcite[!\textit{Zusatztext}\verb!]{!\textit{Verweise}\verb!}!.
\end{itemize}

\noindent%
Für \textit{Verweise} und \textit{Zusatztext} gilt dasselbe wie zuvor erwähnt. Einige Beispiele dazu:

\begin{LaTeXCode}[numbers=none]
	\textcite{Kopka2003} bieten weiterführende Informationen dazu.
\end{LaTeXCode}
$\rightarrow$ \textcite{Kopka2003} bieten weiterführende Informationen dazu.

\begin{LaTeXCode}[numbers=none]
	\textcite[Kap.\ 3]{Kopka2003} beschreiben die Verwendung von \emph{Styles} ausführlich.
\end{LaTeXCode}
$\rightarrow$ \textcite[Kap.\ 3]{Kopka2003} beschreiben die Verwendung von \emph{Styles} ausführlich.

\begin{LaTeXCode}[numbers=none]
	Die von \textcite[S.\ 274--277]{BurgeBurger1999} gemachten Angaben sind falsch.
\end{LaTeXCode}
$\rightarrow$ Die von \textcite[S.\ 274--277]{BurgeBurger1999} gemachten Angaben sind falsch.

\begin{LaTeXCode}[numbers=none]
	Überholt sind auch die Werke von \textcite{BurgeBurger1999, Patashnik1988, Duden1997}.
\end{LaTeXCode}
$\rightarrow$ Überholt sind auch die Werke von \textcite{BurgeBurger1999, Patashnik1988, Duden1997}.

Steht ein Quellenverweis bereits in Klammern, werden diese nicht erneut angegeben und es kommt ein drittes Makro zum Einsatz:
...
\begin{LaTeXCode}[numbers=none]
	Dazu können \emph{Styles} verwendet werden (siehe dazu auch \cite[Kap.\ 3]{Kopka2003}).
\end{LaTeXCode}
$\rightarrow$ Dazu können \emph{Styles} verwendet werden (siehe dazu auch \cite[Kap.\ 3]{Kopka2003}).
...
\begin{center}
\begin{tabular}{rl}
 \textbf{Falsch:}  & \ldots Satzende. \parencite{Oetiker2015} Und weiter \ldots \\
 \textbf{Richtig:} & \ldots Satzende \parencite{Oetiker2015}. Und weiter \ldots
\end{tabular}
\end{center}
...
\begin{center}
\begin{tabular}{rl}
\textbf{Falsch:}  & \ldots Quellenangabe\parencite{Oetiker2015} und weiter \ldots \\
\textbf{Richtig:} & \ldots Quellenangabe \parencite{Oetiker2015} und weiter \ldots
\end{tabular}
\end{center}
...
S.~314 (1632). Zitiert nach \parencite[S.~59]{Hemleben1969}.} % Alle Seitennummern sind frei erfunden!
...
erzeugt. Literatur zur Verwendung von BibTeX findet sich online, \zB \parencite{Feder2006, Patashnik1988}.
...
Typen im \texttt{biblatex}-Handbuch \parencite{Lehman2016} ansehen, beispielsweise
...
\end{GenericCode}
\fullcitebib{OoeRaumordnungsgesetz1994}
...
in \textcite[T=00:32:10]{Psycho1960}.
Alternativ zu einer simplen Zeitangabe "`T=\emph{hh}:\emph{mm}:\emph{ss}"' 
könnte man eine bestimmte Stelle auch auf den Frame genau durch 
den zugehörigen \emph{Timecode} "`TC=\emph{hh:mm:ss:ff}"' angeben, 
\zB\ \parencite[TC=00:32:10:12]{Psycho1960} für Frame \emph{ff}=12.
...
mehrerer Heiliger aufbewahrt werden \parencite{WikiReliquienschrein2016}.
%
...
\end{GenericCode}
\fullcitebib{WikiReliquienschrein2016}
...
\end{GenericCode}
\fullcitebib{HistoryOfComputers2008}
...
wie üblich mit dem \verb!\parencite{..}! Kommando darauf zu verweisen.
...
@online{IBM360,
	...
	title={Plyojump -- Computer History},
...
dieser Vorlage \emph{nicht} mehr verwendet, sondern durch \texttt{biblatex} \parencite{Lehman2016} ersetzt.
...
Durch die Anweisung \verb!\nocite{*}! -- an beliebiger Stelle im Dokument platziert -- werden \emph{alle} bestehenden Einträge der BibTeX-Datei im Quellenverzeichnis aufgelistet, also auch jene, für die es keine explizite \verb!\parencite{}! oder \verb!\textcite{}! Anweisung gibt. Das ist ganz nützlich, um während des Schreibens der Arbeit eine aktuelle Übersicht auszugeben. Normalerweise müssen aber alle angeführten Quellen auch im Text referenziert sein!
